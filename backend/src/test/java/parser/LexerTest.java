...
public class LexerTest {
	...
	public static void main(String... args) throws Exception {
		...
		CharStream input = CharStreams.fromFileName("lexer-test.txt");
		...
		while ((token = lexer.nextToken()).getType() != PmmLexer.EOF) {
			// We get the main.java.semantic value of the token
			Object semanticValue = switch (token.getType()) {
                case PmmLexer.INT_CONSTANT -> LexerHelper.lexemeToInt(token.getText());
                case PmmLexer.CHAR_CONSTANT -> LexerHelper.lexemeToChar(token.getText());
                case PmmLexer.REAL_CONSTANT -> LexerHelper.lexemeToReal(token.getText());
